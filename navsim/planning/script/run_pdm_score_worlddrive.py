...
def run_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[Dict[str, Any]]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        trajectory = agent.compute_trajectory(features, targets, [token])
        if isinstance(trajectory, list):
            pdm_result = pdm_score(
                metric_cache=metric_cache,
                model_trajectory=trajectory[-1],
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
                scorer=scorer,
            )
            ori_pdm_result = pdm_score(
                metric_cache=metric_cache,
                model_trajectory=trajectory[0],
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
                scorer=scorer,
            )
            refine_pdm_score, ori_pdm_score = pdm_result.score, ori_pdm_result.score
